"""详情页翻译脚本 - 并行翻译 content_blocks 和 faq"""
...
def translate_single_tool(tool: dict) -> dict:
    ...
    result = {"tool_id": tool_id, "content_blocks": None, "faq": None, "error": None}
        ...
        if tool["content_html"]:
            try:
                ...
                if blocks and len(blocks) > 0:
                    translated = translate_content_blocks(blocks, "en")
                    result["content_blocks"] = json.dumps(translated, ensure_ascii=False)
        ...
        if tool["faq"]:
            try:
                ...
                if faq and len(faq) > 0:
                    translated = translate_faq(faq, "en")
                    result["faq"] = json.dumps(translated, ensure_ascii=False)
            except (json.JSONDecodeError, TypeError):
                pass
...
def main():
    ...
    cursor.execute("""
        ...
        AND NOT EXISTS (
            ...
            WHERE t.tool_id = d.tool_id AND t.lang = 'en' AND t.field = 'content_blocks'
        )
        ORDER BY d.tool_id
    """)
            ...
            try:
                ...
                if result["error"]:
                    ...
                    if result["content_blocks"]:
                        save_translation(tool_id, "content_blocks", result["content_blocks"])
                    if result["faq"]:
                        save_translation(tool_id, "faq", result["faq"])
                    print(f"[{i+1}/{len(tools_to_translate)}] {tool_id} 完成", flush=True)
                    success += 1
            except Exception as e:
